...
\end{equation}
For $3 \times 3$ kernels across typical intermediate feature depths ($N \in [64, 256]$), this factorized formulation yields an analytical $\approx 8.2\times$ reduction in floating point Multiply-Accumulate (MAC) operations compared to standard dense convolutions.
...
\begin{table}[htbp]
\caption{Quantitative Performance on CamVid Benchmark (100 Epochs)}
\centering
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Evaluation Metric} & \textbf{Training (Epoch 100)} & \textbf{Validation (Epoch 100)} \\
\midrule
Global Pixel Accuracy & $94.62\%$ & $88.05\%$ \\
Dice Similarity Coefficient & $86.85\%$ & $60.13\%$ \\
Mean IoU (mIoU) & $78.62\%$ & $50.08\%$ \\
Categorical Cross-Entropy Loss & $0.2809$ & $0.5380$ \\
\bottomrule
\end{tabular}
\label{tab:camvid_benchmarks}
\end{table}
...
\section{Conclusion}
This study demonstrated that factorized depthwise separable convolutions provide an optimal balance between parameter footprint, computational latency, and spatial segmentation fidelity. On the CamVid autonomous driving benchmark, the proposed architecture achieved $50.08\%$ mIoU and $88.05\%$ pixel accuracy while operating at an $8.2\times$ reduction in computational operations compared to standard convolution baselines.
